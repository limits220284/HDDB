...
type Raft struct {
	...
	LastSnapShotTerm    int   // 全局index，记录snapshot对应的最后一个全局的term
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	// 1、如果任期小于自己的任期，不投票
	// 2、如果任期大于自己的任期，自己的任期需要同步，并且变成follower
	// 3、如果任期等于自己的任期，可以进行投票，但是不需要变成follower
	// 然后比较log索引和任期，必须保证是最新的日志，才能够当选leader
	// 1、如果请求的任期小于自己的，或者任期相等但是日志号小于自己的，不投票
	// 上述条件都没有违背之后，代表可以进行投票
	// 1、更新自己的任期号
	// 2、变成follower
